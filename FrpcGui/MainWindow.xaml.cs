...
namespace FrpcGui
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            try
            {
                ...
                if (App.SilentMode)
                {
                    ...
                    // 显示托盘气泡提示
                    TrayIcon.ShowBalloonTip("ENET客户端", "应用程序已启动并最小化到系统托盘", Hardcodet.Wpf.TaskbarNotification.BalloonIcon.Info);
                }
                else
                {
                    // 正常模式下，显示并激活窗口
                        ...
                        Dispatcher.Invoke(() =>
                        {
                            try
                            {
                                ...
                                if (configNames.Count > 0)
                                {
                                    ...
                                    // 显示通知
                                    TrayIcon.ShowBalloonTip("ENET客户端", $"已自动连接到服务器 ({firstConfig})",
        ...
        {
            if (sender is Button button && button.CommandParameter is Models.ProxyConfig proxy)
            {
        ...
        private void Window_Closing(object sender, CancelEventArgs e)
        {
            ...
            if (!_isExiting)
            {
                ...
                // 显示托盘气泡提示
                TrayIcon.ShowBalloonTip("ENET客户端", "应用程序已最小化到系统托盘", Hardcodet.Wpf.TaskbarNotification.BalloonIcon.Info);
                return;
            }
